...
function updateButtons() {
  ...
  for (const button of document.querySelectorAll('button.service-control')) {
    ...
    button.title = protectedHdfs ? 'Wait for the active upload, download, or top-K job to finish.' : `${button.dataset.action === 'stop' ? 'Stop' : 'Start'} only ${service}`;
  }
}
...
function renderRows(tbody, rows) {
  ...
  if (!rows || !rows.length) {
    tbody.replaceChildren(node('tr', '', node('td', '', 'No rows returned.')));
    return;
  }
...
async function selectFile(file) {
  ...
  await Promise.allSettled([
    ...
    latestTopkForFile(file.path, version),
  ]);
}
async function latestTopkForFile(inputPath, version) {
  try {
...
await Promise.allSettled([refreshStatus(), browse(initialFile ? parent(initialFile) : '/training/top-k/input'), refreshJobs()]);
